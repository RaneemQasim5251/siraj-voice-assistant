    ...
    async def initialize(self):
        ...
        try:
            if PYAUDIO_AVAILABLE:
                ...
                output_device = self._find_best_output_device()
        ...
        try:
            ...
            for i in range(self.pya.get_device_count()):
                ...
                if (info['maxInputChannels'] > 0 and 
                    ...
                    logger.info(f"🎤 Using USB mic: {info['name']}")
                    return i
            ...
            logger.warning(f"⚠️ Error finding input device: {e}")
            return None

    def _find_best_output_device(self):
        """Find best speaker for Pi"""
        if not PYAUDIO_AVAILABLE:
            return None
            
        try:
            # Look for USB audio or HDMI
            for i in range(self.pya.get_device_count()):
                info = self.pya.get_device_info_by_index(i)
                if (info['maxOutputChannels'] > 0 and 
                    ('usb' in info['name'].lower() or 'hdmi' in info['name'].lower())):
                    logger.info(f"🔊 Using audio out: {info['name']}")
                    return i
            
            # Fallback to default
            default = self.pya.get_default_output_device_info()
            logger.info(f"🔊 Using default speaker: {default['name']}")
            return None
            
        except Exception as e:
            logger.warning(f"⚠️ Error finding output device: {e}")
            return None
        ...
        while self.running:
            try:
                if (hasattr(self, 'greeting_requested') and 
                    ...
                    greeting = "السلام عليكم، أهلاً بك، أنا سراج مساعدك في مترو الرياض!"
                    await self.send_greeting(greeting)
